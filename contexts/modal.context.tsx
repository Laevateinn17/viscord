...
export function ModalProvider({ children }: { children: ReactNode }) {
    ...
    const [settingsModal, setSettingsModal] = useState<ModalMetadata>({ type: null });

    function openModal(type: ModalType, data?: any) {
        if (type === ModalType.CHANNEL_SETTINGS || type === ModalType.GUILD_SETTINGS) setSettingsModal({ type, data });
        else setModal({ type, data });
    }

    function closeModal(type?: ModalType) {
        if (type === ModalType.CHANNEL_SETTINGS || type === ModalType.GUILD_SETTINGS) setSettingsModal({ type: null });
    ...
    return (
        <ModalContext.Provider value={{ openModal, closeModal, modal }}>
            ...
            <ChannelSettingsPage channelId={settingsModal.data?.channelId} guildId={settingsModal.data?.guildId} show={settingsModal.type === ModalType.CHANNEL_SETTINGS} onClose={() => closeModal(ModalType.CHANNEL_SETTINGS)} />
            <GuildSettingsPage guildId={settingsModal.data?.guildId} show={settingsModal.type === ModalType.GUILD_SETTINGS} onClose={() => closeModal(ModalType.GUILD_SETTINGS)} />
