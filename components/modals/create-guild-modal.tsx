...
const CreateServerButton = styled.div`
        ...
        <Modal onClose={onClose}>
            <ModalContentContainer className={`${activeContent === ModalContent.Main ? 'active' : ''}`}>
                <ModalHeaderContainer>
                    ...
                    <ModalHeaderText>Your server is where you and your friends hang out. Make yours and start talking.</ModalHeaderText>
                </ModalHeaderContainer>
                <div className="flex flex-col grow px-[24px] mt-[24px]">
                    <CreateServerButton onClick={() => setActiveContent(ModalContent.CreateServer)}>
                        <Image className="m-[16px]" src="/create-my-own-server.svg" width={20} height={20} alt="Create my own" />
                        <p className="grow text-[var(--text-normal)]">Create My Own</p>
                        <FaChevronRight className="mr-[16px]" color="#4f5660" />
                    </CreateServerButton>
                </div>
                <ModalFooterContainer>
                    <h2 className="text-[20px] font-bold mb-[8px] leading-[24px]">Have an invite already?</h2>
                    <JoinServerButton>Join a Server</JoinServerButton>
                </ModalFooterContainer>
            </ModalContentContainer>
            <ModalContentContainer className={`${activeContent === ModalContent.CreateServer ? 'active' : ''}`}>
                <ModalHeaderContainer>
                    <h1 className="text-[24px] font-bold leading-[30px]">Customize Your Server</h1>
                    <ModalHeaderText>Give your new server a personality with a name and an icon. You can always change it later</ModalHeaderText>
                </ModalHeaderContainer>
                <div className="flex flex-col grow px-[24px] mt-[24px]">
                    <UploadImageContainer>
                        <div className="relative w-[80px] h-[80px]">
                            {icon ?
                                <img className="rounded-full w-full h-full" src={URL.createObjectURL(icon)} alt="Server icon" />
                                :
                            ...
                            <input
                                ...
                                className="absolute min-h-full w-full top-0 cursor-pointer opacity-0" />
                        </div>
                    </UploadImageContainer>
                    <TextInput
                        label="Server Name"
